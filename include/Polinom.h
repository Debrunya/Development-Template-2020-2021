...
Polinom::Polinom(TList<int, float>* list)
{
    ...
    while (!list->IsEnded())
    {
        if (list->GetCurrent()->key > 7999 || list->GetCurrent()->key < 0)
    ...
    while (i < str.length())
    {
        do
        {
            ...
            if (is_x && isdigit(str[i]))
            {
                if (isdigit(str[i + 1]))
                {
                    deg_x = ((int)str[i] - 48) * 10 + ((int)str[i + 1] - 48);
                    if (deg_x > 19) throw "degree > 19";
                    i++;
                }
                else deg_x = (int)str[i] - 48;
                is_x = false;
                i++;
                continue;
            }
            ...
            if (is_y && isdigit(str[i]))
            {
                if (isdigit(str[i + 1]))
                {
                    deg_y = ((int)str[i] - 48) * 10 + ((int)str[i + 1] - 48);
                    if (deg_y > 19) throw "degree > 19";
                    i++;
                }
                else deg_y = (int)str[i] - 48;
                is_y = false;
                i++;
                continue;
            }
            ...
            if (is_z && isdigit(str[i]))
            {
                if (isdigit(str[i + 1]))
                {
                    deg_z = ((int)str[i] - 48) * 10 + ((int)str[i + 1] - 48);
                    if (deg_z > 19) throw "degree > 19";
                    i++;
                }
                else deg_z = (int)str[i] - 48;
                is_z = false;
                i++;
                continue;
            }
...
Polinom Polinom::operator+(const Polinom& polinom)const
{
    if (monoms->IsEmpty())
        return polinom;
    if (polinom.monoms->IsEmpty())
        return *this;
    Polinom sum;
    Polinom pol1(*this);
    Polinom pol2(polinom);
    pol1.monoms->Reset();
    pol2.monoms->Reset();
...
bool Polinom::operator==(const Polinom& polinom) const
{
    ...
    TNode<int, float>* second = polinom.monoms->pFirst;
...
const Polinom& Polinom::operator=(const Polinom& polinom)
{
    if (*this == polinom)
        return *this;
    delete monoms;
    monoms = new TList<int, float>(*polinom.monoms);
    return *this;
}
...
ostream& operator <<(std::ostream& out, const Polinom& pol)
{
    ...
    while (!pol.monoms->IsEnded())
    {
        int deg_x = pol.monoms->GetCurrent()->key / pow(20,2);
        int deg_y = (pol.monoms->GetCurrent()->key - deg_x * pow(20, 2)) / 20;
        int deg_z = pol.monoms->GetCurrent()->key - deg_x * pow(20, 2) - deg_y * 20;
